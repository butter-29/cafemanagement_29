...
app.config['MYSQL_PASSWORD'] = 'Mukesh'      #password
...
@app.route("/coffeemenu", methods=["GET", "POST"])
def coffeemenu():
    if request.method == "POST":
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('''
            INSERT INTO ORDERS (ITEM_NAME, ITEM_NO, MOB_NO, QUANTITY)
            VALUES (%s, %s, %s, %s)
        ''', (item_name, item_no, customer_mobile, quantity))
        mysql.connection.commit()

        flash(f'Order placed for {item_name} (x{quantity})!')
